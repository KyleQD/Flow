...
export default async function ForumPage({ params, searchParams }: { params: { slug: string }, searchParams?: { sort?: string, page?: string } }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-300">
        <span>Sort:</span>
        <Link href={`/forums/${params.slug}?sort=hot`} className={`px-2 py-1 rounded-lg ${sort === 'hot' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}>Hot</Link>
        <Link href={`/forums/${params.slug}?sort=new`} className={`px-2 py-1 rounded-lg ${sort === 'new' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}>New</Link>
        <Link href={`/forums/${params.slug}?sort=top`} className={`px-2 py-1 rounded-lg ${sort === 'top' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}>Top</Link>
      </div>
      </div>
      <div className="space-y-3">
        {(threads || []).map(t => (
          <Link key={t.id} href={`/forums/${params.slug}/thread/${t.id}`} className="block bg-slate-900/50 border border-slate-800/60 rounded-2xl p-4 hover:border-purple-500/40">
      ...
      <div className="flex items-center justify-between pt-2">
        {hasPrev ? (
          <Link href={`/forums/${params.slug}?sort=${encodeURIComponent(sort)}&page=${page - 1}`} className="text-sm px-3 py-2 rounded-xl bg-slate-800 text-slate-200 hover:bg-slate-700">Previous</Link>
        ) : <div />}
        {hasNext && (
          <Link href={`/forums/${params.slug}?sort=${encodeURIComponent(sort)}&page=${page + 1}`} className="text-sm px-3 py-2 rounded-xl bg-slate-800 text-slate-200 hover:bg-slate-700">Next</Link>
        )}
      </div>
    </div>
  )
