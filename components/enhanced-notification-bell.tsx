...
const notificationIcons: Record<string, { icon: React.ComponentType<any>; color: string; bgColor: string }> = {
  follow_request: { icon: User, color: "text-blue-400", bgColor: "rgba(59, 130, 246, 0.1)" },
  follow_accepted: { icon: CheckCircle, color: "text-green-400", bgColor: "rgba(34, 197, 94, 0.1)" },
  like: { icon: Heart, color: "text-red-400", bgColor: "rgba(239, 68, 68, 0.1)" },
  comment: { icon: MessageSquare, color: "text-yellow-400", bgColor: "rgba(234, 179, 8, 0.1)" },
  event_invite: { icon: Calendar, color: "text-purple-400", bgColor: "rgba(168, 85, 247, 0.1)" },
  default: { icon: Bell, color: "text-gray-400", bgColor: "rgba(107, 114, 128, 0.1)" }
}
          ...
          >
            ...
            {unreadCount > 0 && (
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center border-2 border-slate-900"
              >
              ...
              {isLoading ? (
                ...
                <AnimatePresence>
                  {notifications.map((notification) => {
                    ...
                    const Icon = iconConfig.icon as React.ComponentType<any>
                    ...
                    return (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className={`p-3 rounded-lg mb-2 cursor-pointer transition-all duration-200 hover:bg-slate-800/50 ${
