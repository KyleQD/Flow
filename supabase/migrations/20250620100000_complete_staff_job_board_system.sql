-- =============================================================================
-- COMPLETE STAFF MANAGEMENT & JOB BOARD SYSTEM MIGRATION
-- This migration enhances existing tables and adds new ones for our full system
-- Preserves all existing data while adding comprehensive functionality
-- =============================================================================
...
-- =============================================================================
-- ENHANCE EXISTING STAFF_JOBS TABLE
-- =============================================================================

-- Add new columns to existing staff_jobs table
DO $$
BEGIN
  -- Add venue integration if not exists
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_jobs' AND column_name = 'salary_range_min') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_jobs' AND column_name = 'deadline') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_jobs' AND column_name = 'applications_count') THEN
    ALTER TABLE staff_jobs ADD COLUMN applications_count INTEGER DEFAULT 0;
  END IF;

  RAISE NOTICE '✅ Enhanced staff_jobs table with new fields';
END $$;

-- =============================================================================
-- ENHANCE EXISTING STAFF_APPLICATIONS TABLE  
-- =============================================================================

-- Add new columns to existing staff_applications table
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_applications' AND column_name = 'applicant_name') THEN
  ...
  RAISE NOTICE '✅ Enhanced staff_applications table with new fields';
END $$;
...
-- Staff Documents Policies (Private)
...
-- Job Board Indexes
CREATE INDEX IF NOT EXISTS idx_staff_jobs_venue_id ON staff_jobs(venue_id);
CREATE INDEX IF NOT EXISTS idx_staff_jobs_status ON staff_jobs(status);
CREATE INDEX IF NOT EXISTS idx_staff_jobs_priority ON staff_jobs(priority);
CREATE INDEX IF NOT EXISTS idx_staff_jobs_department ON staff_jobs(department);
CREATE INDEX IF NOT EXISTS idx_staff_jobs_deadline ON staff_jobs(deadline);
...
CREATE INDEX IF NOT EXISTS idx_staff_applications_rating ON staff_applications(rating);
CREATE INDEX IF NOT EXISTS idx_staff_applications_ai_match_score ON staff_applications(ai_match_score);
...
BEGIN
  ...
  INTO app_skills, app_experience
  ...
  availability_score := CASE 
    WHEN availability = 'Immediate' THEN 20
    WHEN availability LIKE '%week%' THEN 15
    ELSE 10
  END;
...
BEGIN
  RAISE NOTICE '=================================================';
  RAISE NOTICE '🚀 COMPLETE STAFF MANAGEMENT SYSTEM SETUP DONE!';
  RAISE NOTICE '=================================================';
  RAISE NOTICE '📊 Enhanced Tables:';
  RAISE NOTICE '   ✅ staff_jobs (added venue_id, priority, salary_range, etc.)';
  RAISE NOTICE '   ✅ staff_applications (added rating, ai_match_score, etc.)';
  RAISE NOTICE '';
