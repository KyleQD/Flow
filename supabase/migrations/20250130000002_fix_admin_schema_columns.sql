-- =============================================================================
-- FIX ADMIN SCHEMA COLUMN ISSUES
-- This migration fixes column mismatches and adds missing columns
-- Migration: 20250130000002_fix_admin_schema_columns.sql
-- =============================================================================

-- Check and add missing columns to tours table
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tours' AND column_name = 'created_by') THEN
    ALTER TABLE tours ADD COLUMN created_by UUID REFERENCES profiles(id);
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'venue_id') THEN
    ALTER TABLE events ADD COLUMN venue_id UUID REFERENCES venue_profiles(id) ON DELETE SET NULL;
  END IF;
