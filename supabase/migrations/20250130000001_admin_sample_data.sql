-- =============================================================================
-- SAMPLE DATA FOR ADMIN PLATFORM TESTING
-- This migration adds sample data to test the admin platform functionality
-- Migration: 20250130000001_admin_sample_data.sql
-- =============================================================================

-- Sample tours data
INSERT INTO tours (name, description, status, start_date, end_date, total_shows, completed_shows, budget, expenses, revenue, transportation, accommodation, equipment_requirements, crew_size, created_by) VALUES
('Summer Festival Tour 2024', 'A nationwide summer festival tour featuring multiple artists', 'active', '2024-06-01', '2024-08-31', 25, 8, 500000.00, 125000.00, 750000.00, 'Bus and van transportation', 'Hotel accommodations', 'Full sound and lighting rig', 12, (SELECT id FROM profiles LIMIT 1)),
('Winter Arena Tour', 'Large arena tour across major cities', 'planning', '2024-12-01', '2025-02-28', 15, 0, 750000.00, 0.00, 0.00, 'Private jet and buses', 'Luxury hotels', 'Advanced stage setup', 20, (SELECT id FROM profiles LIMIT 1)),
('Club Circuit 2024', 'Small venue club tour', 'completed', '2024-03-01', '2024-05-31', 30, 30, 150000.00, 140000.00, 300000.00, 'Van transportation', 'Budget hotels', 'Basic sound system', 6, (SELECT id FROM profiles LIMIT 1))
ON CONFLICT DO NOTHING;

-- Sample events data
INSERT INTO events (name, description, tour_id, venue_name, venue_address, event_date, event_time, doors_open, duration_minutes, status, capacity, tickets_sold, ticket_price, vip_price, expected_revenue, actual_revenue, expenses, sound_requirements, lighting_requirements, stage_requirements, venue_contact_name, venue_contact_email, venue_contact_phone, created_by) VALUES
('Summer Festival - Los Angeles', 'Opening night of the summer festival tour', (SELECT id FROM tours WHERE name = 'Summer Festival Tour 2024' LIMIT 1), 'LA Festival Grounds', '123 Festival Way, Los Angeles, CA', '2024-06-15', '20:00:00', '18:00:00', 180, 'confirmed', 5000, 4200, 75.00, 150.00, 375000.00, 315000.00, 25000.00, 'Full PA system with subs', 'LED lighting rig', 'Large outdoor stage', 'John Smith', 'john@lafestival.com', '555-0123', (SELECT id FROM profiles LIMIT 1)),
('Summer Festival - New York', 'Second stop on the summer festival tour', (SELECT id FROM tours WHERE name = 'Summer Festival Tour 2024' LIMIT 1), 'NYC Central Park', '456 Park Avenue, New York, NY', '2024-06-22', '19:30:00', '17:30:00', 180, 'confirmed', 8000, 7200, 85.00, 175.00, 680000.00, 612000.00, 35000.00, 'Large outdoor PA system', 'Full lighting production', 'Main stage setup', 'Sarah Johnson', 'sarah@nycparks.com', '555-0456', (SELECT id FROM profiles LIMIT 1)),
('Winter Arena - Chicago', 'First arena show of the winter tour', (SELECT id FROM tours WHERE name = 'Winter Arena Tour' LIMIT 1), 'Chicago Arena', '789 Arena Blvd, Chicago, IL', '2024-12-15', '20:00:00', '18:00:00', 240, 'scheduled', 15000, 0, 95.00, 200.00, 1425000.00, 0.00, 0.00, 'Arena sound system', 'Full arena lighting', 'Large arena stage', 'Mike Wilson', 'mike@chicagoarena.com', '555-0789', (SELECT id FROM profiles LIMIT 1))
ON CONFLICT DO NOTHING;

-- Sample staff profiles data
...
((SELECT id FROM auth.users LIMIT 1), 'EMP001', 'Alex', 'Johnson', 'alex.johnson@tourify.com', '555-0101', 'Tour Manager', 'Management', '2023-01-15', 'full_time', 75000.00, NULL, 'active', 'available', ARRAY['project_management', 'logistics', 'team_leadership'], ARRAY['Tour Management Certification'], 4.8, 15, 'Los Angeles, CA'),
((SELECT id FROM auth.users LIMIT 1), 'EMP002', 'Maria', 'Garcia', 'maria.garcia@tourify.com', '555-0102', 'Sound Engineer', 'Technical', '2023-03-20', 'full_time', 65000.00, NULL, 'active', 'available', ARRAY['sound_mixing', 'equipment_maintenance', 'live_audio'], ARRAY['Audio Engineering Certification'], 4.9, 12, 'New York, NY'),
((SELECT id FROM auth.users LIMIT 1), 'EMP003', 'David', 'Chen', 'david.chen@tourify.com', '555-0103', 'Lighting Technician', 'Technical', '2023-02-10', 'full_time', 60000.00, NULL, 'active', 'available', ARRAY['lighting_design', 'equipment_setup', 'programming'], ARRAY['Lighting Design Certification'], 4.7, 18, 'Chicago, IL'),
((SELECT id FROM auth.users LIMIT 1), 'EMP004', 'Sarah', 'Williams', 'sarah.williams@tourify.com', '555-0104', 'Stage Manager', 'Operations', '2023-04-05', 'full_time', 70000.00, NULL, 'active', 'available', ARRAY['stage_management', 'scheduling', 'safety'], ARRAY['Stage Management Certification'], 4.6, 10, 'Austin, TX'),
((SELECT id FROM auth.users LIMIT 1), 'EMP005', 'James', 'Brown', 'james.brown@tourify.com', '555-0105', 'Security Coordinator', 'Security', '2023-01-30', 'full_time', 55000.00, NULL, 'active', 'available', ARRAY['crowd_control', 'safety_protocols', 'emergency_response'], ARRAY['Security Management Certification'], 4.5, 8, 'Nashville, TN')
ON CONFLICT DO NOTHING;
...
-- Sample artist profiles data (if user_id exists)
DO $$
DECLARE
    user_id_exists UUID;
BEGIN
    ...
    IF user_id_exists IS NOT NULL THEN
        INSERT INTO artist_profiles (user_id, artist_name, bio, genres, social_links, verification_status, account_tier) VALUES
        (user_id_exists, 'The Midnight Runners', 'Alternative rock band from Los Angeles', ARRAY['alternative_rock', 'indie'], '{"spotify": "https://spotify.com/artist/midnightrunners", "instagram": "https://instagram.com/midnightrunners"}', 'verified', 'premium'),
        (user_id_exists, 'DJ Luna', 'Electronic music producer and DJ', ARRAY['electronic', 'house', 'techno'], '{"spotify": "https://spotify.com/artist/djluna", "soundcloud": "https://soundcloud.com/djluna"}', 'verified', 'pro'),
        (user_id_exists, 'Marcus Williams Quartet', 'Jazz ensemble featuring Marcus Williams', ARRAY['jazz', 'smooth_jazz'], '{"spotify": "https://spotify.com/artist/marcuswilliams", "youtube": "https://youtube.com/marcuswilliams"}', 'pending', 'basic')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;

-- Sample venue profiles data (if user_id exists)
DO $$
DECLARE
    user_id_exists UUID;
BEGIN
    SELECT id INTO user_id_exists FROM auth.users LIMIT 1;
    IF user_id_exists IS NOT NULL THEN
        INSERT INTO venue_profiles (user_id, venue_name, description, address, city, state, country, capacity, venue_types, contact_info, social_links, verification_status, account_tier) VALUES
        (user_id_exists, 'LA Festival Grounds', 'Large outdoor festival venue in Los Angeles', '123 Festival Way', 'Los Angeles', 'CA', 'USA', 5000, ARRAY['outdoor', 'festival'], '{"phone": "555-0201", "email": "info@lafestival.com"}', '{"website": "https://lafestival.com", "instagram": "https://instagram.com/lafestival"}', 'verified', 'premium'),
        (user_id_exists, 'NYC Central Park', 'Historic outdoor venue in Central Park', '456 Park Avenue', 'New York', 'NY', 'USA', 8000, ARRAY['outdoor', 'park'], '{"phone": "555-0202", "email": "events@nycparks.com"}', '{"website": "https://nycparks.com", "facebook": "https://facebook.com/nycparks"}', 'verified', 'premium'),
        (user_id_exists, 'Chicago Arena', 'Large indoor arena in downtown Chicago', '789 Arena Blvd', 'Chicago', 'IL', 'USA', 15000, ARRAY['arena', 'indoor'], '{"phone": "555-0203", "email": "booking@chicagoarena.com"}', '{"website": "https://chicagoarena.com", "twitter": "https://twitter.com/chicagoarena"}', 'verified', 'pro')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;

-- Sample equipment data
INSERT INTO equipment (name, category, description, serial_number, condition, location, purchase_date, purchase_cost, status) VALUES
('Midas M32 Console', 'sound', '32-channel digital mixing console', 'M32-001', 'excellent', 'Los Angeles Warehouse', '2023-01-15', 8500.00, 'available'),
('JBL SRX828SP Subwoofers', 'sound', 'Dual 18" powered subwoofers', 'SRX828-001', 'good', 'Los Angeles Warehouse', '2023-02-20', 2400.00, 'available'),
('Martin MAC Aura XB', 'lighting', 'LED moving head wash fixtures', 'MACAURA-001', 'excellent', 'New York Warehouse', '2023-03-10', 3200.00, 'available'),
('Chauvet Rogue R2 Wash', 'lighting', 'Moving head wash fixtures', 'ROGUE-001', 'good', 'Chicago Warehouse', '2023-04-05', 2800.00, 'available'),
('Truss System 40ft', 'stage', '40-foot aluminum truss system', 'TRUSS-001', 'good', 'Los Angeles Warehouse', '2023-01-30', 15000.00, 'available')
ON CONFLICT DO NOTHING;
...
VALUES ('SAMPLE_DATA_CREATED', 'migration', NULL, '{"migration": "20250130000001_admin_sample_data.sql", "tables_populated": "all_admin_tables", "status": "success"}'); 