-- Create workflow tables for onboarding system
-- Migration: 20250123000001_workflow_system.sql
...
CREATE TABLE IF NOT EXISTS onboarding_workflows (
  ...
  current_stage VARCHAR(50) NOT NULL DEFAULT 'job_posted',
...
-- Users can view workflows for venues they have access to
CREATE POLICY "Users can view venue workflows" ON onboarding_workflows
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM venue_members vm
      WHERE vm.venue_id = onboarding_workflows.venue_id
      AND vm.user_id = auth.uid()
    )
  );

-- Users can insert workflows for venues they have access to
...
-- Users can update workflows for venues they have access to
...
-- Users can delete workflows for venues they have access to
CREATE POLICY "Users can delete venue workflows" ON onboarding_workflows
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM venue_members vm
      WHERE vm.venue_id = onboarding_workflows.venue_id
...
CREATE POLICY "Users can view their notifications" ON notifications
  FOR SELECT USING (
    ...
    EXISTS (
      SELECT 1 FROM venue_members vm
      WHERE vm.venue_id = notifications.venue_id
      AND vm.user_id = auth.uid()
    )
  );

-- Users can create notifications for venues they have access to
