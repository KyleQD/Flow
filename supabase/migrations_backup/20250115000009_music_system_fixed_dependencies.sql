-- =============================================
-- Music System Migration (Fixed Dependencies)
-- =============================================
-- This migration creates the complete music upload and sharing system
-- with proper dependency handling and error checking
...
-- Check if artist_profiles table exists, if not create a basic version
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    -- Create basic artist_profiles table if it doesn't exist
    ...
    RAISE NOTICE 'Created basic artist_profiles table';
  ELSE
    RAISE NOTICE 'artist_profiles table already exists';
  END IF;
END $$;

-- =============================================
-- CLEANUP EXISTING OBJECTS (SAFE)
-- =============================================
...
-- Drop triggers safely (only if tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'music_likes') THEN
    DROP TRIGGER IF EXISTS trigger_update_music_likes_count ON music_likes;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'music_comments') THEN
    DROP TRIGGER IF EXISTS trigger_update_music_comments_count ON music_comments;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'music_plays') THEN
    DROP TRIGGER IF EXISTS trigger_update_music_plays_count ON music_plays;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'music_comment_likes') THEN
...
WHERE table_name IN ('artist_music', 'music_likes', 'music_comments', 'music_comment_likes', 'music_shares', 'music_plays')
AND table_schema = 'public';
