-- Admin Onboarding & Staff Management Database Setup
...
-- Create simple RLS policies that allow authenticated users to access their own data
-- These policies can be customized based on your specific venue/user relationship logic

-- Job Posting Templates - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view job postings" ON job_posting_templates
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert job postings" ON job_posting_templates
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update their own job postings" ON job_posting_templates
    FOR UPDATE USING (auth.uid() = created_by);

-- Job Applications - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view applications" ON job_applications
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert applications" ON job_applications
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update applications they reviewed" ON job_applications
    FOR UPDATE USING (auth.uid() = reviewed_by OR auth.uid() = applicant_id);

-- Onboarding Workflows - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view workflows" ON onboarding_workflows
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert workflows" ON onboarding_workflows
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update their own workflows" ON onboarding_workflows
    FOR UPDATE USING (auth.uid() = created_by);

-- Onboarding Steps - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view steps" ON onboarding_steps
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert steps" ON onboarding_steps
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- Staff Onboarding Candidates - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view candidates" ON staff_onboarding_candidates
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert candidates" ON staff_onboarding_candidates
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update candidates they approved" ON staff_onboarding_candidates
    FOR UPDATE USING (auth.uid() = approved_by OR auth.uid() = user_id);

-- Onboarding Activities - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view activities" ON onboarding_activities
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert activities" ON onboarding_activities
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update activities they completed" ON onboarding_activities
    FOR UPDATE USING (auth.uid() = completed_by);

-- Staff Members - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view staff members" ON staff_members
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert staff members" ON staff_members
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update their own staff records" ON staff_members
    FOR UPDATE USING (auth.uid() = user_id);

-- Staff Messages - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view messages" ON staff_messages
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert messages" ON staff_messages
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow users to update their own messages" ON staff_messages
    FOR UPDATE USING (auth.uid() = sender_id);

-- Application Form Templates - Allow authenticated users to view all (can be restricted later)
CREATE POLICY "Allow authenticated users to view form templates" ON application_form_templates
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to insert form templates" ON application_form_templates
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');
