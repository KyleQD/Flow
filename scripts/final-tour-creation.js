...
async function createSuperAwesomeTour() {
  ...
  try {
    ...
    // Navigate to next step
    const nextStepButtons = await page.$$('button');
    ...
    // Navigate to next step
    const nextEventButtons = await page.$$('button');
    let nextEventButton = null;
    
    for (const button of nextEventButtons) {
      const text = await button.evaluate(el => el.textContent);
      if (text && text.toLowerCase().includes('next')) {
    ...
    // Navigate to next step
    const nextCrewButtons = await page.$$('button');
    let nextCrewButton = null;
    
    for (const button of nextCrewButtons) {
      const text = await button.evaluate(el => el.textContent);
      if (text && text.toLowerCase().includes('next')) {
        nextCrewButton = button;
        break;
      }
    }
    ...
    // Navigate to next step
    const nextLogisticsButtons = await page.$$('button');
    let nextLogisticsButton = null;
    
    for (const button of nextLogisticsButtons) {
      const text = await button.evaluate(el => el.textContent);
      if (text && text.toLowerCase().includes('next')) {
        nextLogisticsButton = button;
        break;
      }
    }
    ...
    // Navigate to next step
    const nextFinancialButtons = await page.$$('button');
    let nextFinancialButton = null;
    
    for (const button of nextFinancialButtons) {
      const text = await button.evaluate(el => el.textContent);
      if (text && text.toLowerCase().includes('next')) {
        nextFinancialButton = button;
        break;
      }
    }
