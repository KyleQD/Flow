-- Migration: Achievement, Badge, and Endorsement System (FIXED)
-- Description: Comprehensive system for tracking user accomplishments, achievements, and peer endorsements
-- Fixed: Removed references to non-existent profile_visibility column
...
-- FIXED: Use account_settings instead of profile_visibility
CREATE POLICY "Users can view public badges" ON user_badges
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE profiles.id = user_badges.user_id 
      AND (profiles.account_settings->'privacy'->>'profile_public')::boolean = true
    )
  );
